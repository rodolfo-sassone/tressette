...
class InferenceMethod(Displayable):
    ...
    def show_post(self, obs={}, format_string="{:.3f}"):
        """draws the graphical model conditioned on observations obs
           format_string is changed if you want more or less precision 
        """
        plt.ion()   # interactive
        ax = plt.figure().gca()
        ax.set_axis_off()
        plt.title(self.gm.title+" observed: "+str(obs))
        bbox = dict(boxstyle="round4,pad=1.0,rounding_size=0.5")
        for var in reversed(self.gm.topological_sort()):
            distn = self.query(var, obs=obs)
            if var in obs:
                text =  var.name + "=" + str(obs[var])
            else:
                text = var.name + "\n" + "\n".join(str(d)+": "+format_string.format(v) for (d,v) in distn.items())
            if self.gm.var2parents[var]:
                ...
                x,y = var.position
                plt.text(x,y,text,bbox=bbox,ha='center')
